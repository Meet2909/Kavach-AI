...
def get_loaded_models(host: str = OLLAMA_URL):
    ...
    try:
        ...
        print(f"Error checking loaded models on {host}: {e}")
    return []

def unload_model(model_name: str, host: str = OLLAMA_URL):
    """Forcefully unloads a specific model from VRAM to free up the 6GB capacity."""
    print(f"Unloading model: {model_name} on {host}...")
    data = json.dumps({
        "model": model_name,
        "keep_alive": 0
    }).encode('utf-8')
...
def swap_model(target_model: str, host: str = OLLAMA_URL):
    """Ensures the target_model is the only one loaded in VRAM."""
    print(f"--- Initiating Hot-Swap to {target_model} on {host} ---")
    ...
    loaded_models = get_loaded_models(host)
